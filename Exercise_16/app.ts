...
let guests: string[] = ["Ali", "Omar", "Youssef"];
...
let cantMakeIt: string = "omer";
let indexOfCantMakeIt: number = guests.indexOf(cantMakeIt);
if (indexOfCantMakeIt !== -1) {
  guests.splice(indexOfCantMakeIt, 1);
}
// Add a new guest
let newGuest: string = "Ibrahim";
...
for (let i = 0; i < guests.length; i++) {
  console.log(
    `Dear ${guests}, you are cordially invited to dinner. Please join us for an evening of great food and conversation.`
  );
}
// Inform that a bigger table is available
console.log("\nGood news! We found a bigger dinner table!");
// Add new guests
let newGuests: string[] = ["Khaled", "Hassan", "Karim"];
...
for (let i = 0; i < guests.length; i++) {
  console.log(
    `Dear ${guests[i]}, you are cordially invited to dinner. Please join us for an evening of great food and conversation.`
  );
}
